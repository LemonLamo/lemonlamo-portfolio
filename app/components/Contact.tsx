...
const contactMethods = [
  ...
  },
  {
    ...
    href: 'https://medium.com/@lamia.koucem',
    icon: (
      <svg className="w-6 h-6" fill="currentColor" viewBox="0 0 24 24">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {contactMethods.map((method, index) => (
              <a
                ...
                className={`group relative flex flex-col items-center p-5 rounded-3xl transition-all duration-500 overflow-hidden ${hoveredIndex === index ? 'scale-105' : 'scale-100'}`}
                style={{
                  backgroundColor: 'var(--card-bg)',
                  border: '2px solid',
                  borderColor: method.type === 'Email' || method.type === 'GitHub' ? 'var(--accent-pink)' : 'var(--accent-purple)',
                  boxShadow: hoveredIndex === index ? '0 20px 50px rgba(139, 92, 246, 0.2)' : 'none'
                }}
              >
                {/* Content */}
                <div className="relative z-10 flex flex-col items-center">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-3 transform transition-all duration-500 ${hoveredIndex === index ? 'scale-110' : 'scale-100'}`}
                    style={{
                      backgroundColor: method.type === 'Email' || method.type === 'GitHub' ? 'rgba(236, 72, 153, 0.15)' : 'rgba(139, 92, 246, 0.15)',
                      color: method.type === 'Email' || method.type === 'GitHub' ? 'var(--accent-pink)' : 'var(--accent-purple)'
                    }}>
                    {method.icon}
                  </div>
                  <h3 className="font-semibold text-base mb-1.5 transition-colors duration-500"
